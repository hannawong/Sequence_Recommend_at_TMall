        ...
        for itr in range(10):
            ...
            while True:
                ...
                if not train_data_pool.empty():
                    ...
                    continue
                #print (src[0]) ###(245176,244462...),user_id: length = 256(batchsize)
                #print (src[1]) ##[53745,...46409],item_id: length = 256(batchsize)
                #print (src[2]) ##[251638,249264...],cate_id: length = 256(batchsize)
                #print(tgt[0]) ##[0,1]...##label
                #print(tgt[1]) ##hist_item, 256(batchsize) * 200(max_history_len) 
                #print(tgt[2]) ##hist_cate, 256(batchsize) * 200(max_history_len) 
                #print(tgt[3]) ##neg_item,256(batchsize) * 200(max_history_len) 
                #print(tgt[4]) ##neg_cate,256(batchsize) * 200(max_history_len) 
                
                #print(len(tgt[5][0])) ##hist_mask??
                #print("====")
                nick_id, item_id, cate_id, label, hist_item, hist_cate, neg_item, neg_cate, hist_mask = prepare_data(src, tgt)
...
if __name__ == '__main__':
    ...
    Memory_Size = args.memory_size ##4
    Mem_Induction = args.mem_induction ##0
    Util_Reg = args.util_reg ##0
    print(Model_Type,"!!!!!!!!") 
