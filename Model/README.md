...
sh prepare_tmall.sh
```
in directory /Model/preprocess. This would give you `taobao_feature.pkl`,`taobao_test.txt` and `taobao_train.txt`. 
...
- Moreover, now I only use **random** negative sampling, which is the easiest way to perform negative sampling. However, this might make things too easy. We can also try other ways to do negative sampling, such as *hard negative sampling*, and negative sampling based on *streaming frequency estimation*. ([Recsys 2019, Google](https://research.google/pubs/pub48840/))
